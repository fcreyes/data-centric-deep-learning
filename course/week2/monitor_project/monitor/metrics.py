...
def get_ks_score(tr_probs, te_probs):
  ...
  _, score = ks_2samp(tr_probs, te_probs)
  # ============================
  return score
...
def get_hist_score(tr_probs, te_probs, bins=10):
  ...
  tr_heights, bin_edges = np.histogram(tr_probs, density=True)
...
def get_vocab_outlier(tr_vocab, te_vocab):
  ...
  # score: float (between 0 and 1)
  intersect_keys = list(tr_vocab.keys() & te_vocab.keys())

  if len(intersect_keys) == 0:
    return 1

  total_test_words = sum(te_vocab.values())
  test_words_in_train = sum([te_vocab[k] for k in intersect_keys])
  score = 1 - (test_words_in_train / total_test_words)
  # ============================
  return score
...
class MonitoringSystem:
  ...
  def calibrate(self, tr_probs, tr_labels, te_probs):
    ...
    model.fit(tr_probs, tr_labels)
    tr_probs_cal = torch.Tensor(model.predict(tr_probs))
    te_probs_cal = torch.Tensor(model.predict(te_probs))
    # ============================
